export class Minimap {
    ...
    drawPlayer(pos, rotationAngle, color = '#00ffff') {
        ...
        // Use the passed color for the cone, with transparency
        gradient.addColorStop(0, `${color}40`); // 25% opacity (approx) - using hex alpha if 8-char hex provided or fallback
        // Actually, let's assume color is 7 char hex (#RRGGBB).
        // If it's #RRGGBB, we can append hex alpha.
        // Let's use a helper or just append if we know format.
        // The themeColor is passed from levelColors which are #RRGGBB.

        // Let's perform a simple check or just work with rgba by converting hex.
        // Or simpler: verify themeColor format. It's '#00ffff' etc from this.levelColors.

        gradient.addColorStop(0, color + '40'); // ~25%
